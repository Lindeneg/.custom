...
const formReducer: ValidationReducer<
  State,
  ValidationAction<FormActions, Payload>
> = (state, action) => {
  const pl: Payload = action.payload;
  switch (action.type) {
    case FormActions.INPUT_CHANGE:
      if (
        typeof pl.id !== 'undefined' &&
        typeof pl.isValid !== 'undefined' &&
        typeof pl.value !== 'undefined'
      ) {
...
export const useForm = (
  initialState: State
): [State, InputHandler, SetFormState] => {
  ...
  const inputHandler: InputHandler = useCallback(
    (id: string, value: ValidationValue, isValid: boolean): void => {
